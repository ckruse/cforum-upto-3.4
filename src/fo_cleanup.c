 ...
 */
int cleanup_files() {
  t_name_value *v,*forums = cfg_get_first_value(&fo_server_conf,NULL,"Forums");
  ...
  for(i=0;i<forums->valnum;++i) {
    v = cfg_get_first_value(&fo_default_conf,forums->values[i],"ConfigDirectory");
    udir = opendir(v->values[0]);
    len = strlen(v->values[0]);

    cf_hash_set(GlobalValues,"FORUM_NAME",10,forums->values[i],strlen(forums->values[i]));
...
/* {{{ cleanup_votes */
void cleanup_votes() {
  t_name_value *forums = cfg_get_first_value(&fo_server_conf,NULL,"Forums"),*v;
  size_t i,len;
  u_char *line;
  rline_t rsd;
  t_cf_tree tree;
  ...
  for(i=0;i<forums->valnum;++i) {
    v = cfg_get_first_value(&fo_vote_conf,forums->values[i],"VotingDatabase");
...
int main(int argc,char *argv[],char *envp[]) {
  ...
  t_array *cfgfiles;
  u_char *file;
  t_configfile conf,dconf,vconf,sconf;

  static const u_char *wanted[] = {
    "fo_default","fo_view","fo_vote","fo_server"
  };
  ...
  if((cfgfiles = get_conf_file(wanted,4)) == NULL) {
  ...
  file = *((u_char **)array_element_at(cfgfiles,3));
  ...
  if(read_config(&dconf,ignre,CFG_MODE_CONFIG|CFG_MODE_NOLOAD) != 0 ||
     ...
     read_config(&sconf,ignre,CFG_MODE_CONFIG|CFG_MODE_NOLOAD) != 0) {
