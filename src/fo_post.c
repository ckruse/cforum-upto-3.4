  ...
  if(*field->content != 0xC3 || *(field->content+1) != 0xBF) {
    ...
    for(key=head->keys.elems;key;key=key->next) {
      for(param = cf_cgi_get_multiple(head,key->key);param;param=param->next) {
        ...
        cf_str_init(&str);
        len = param->value.len;
        ptr = param->value.content;
        do {
          ptr += i = utf8_to_unicode(ptr,len-(int)(ptr-param->value.content),&num);
          if(cf_isspace(num) && num != 0x9 && num != 012 && num != 015) cf_str_char_append(&str,' ');
          else {
            i = unicode_to_utf8(num,ubuff,10);
            cf_str_chars_append(&str,ubuff,i);
          }
        } while(*ptr);
        /* }}} */
...
cf_string_t *body_plain2coded(const u_char *qchars_p,const u_char *text) {
   ...
   * - change more than one whitespace at the beginning and in the
   *   middle of the string to &nbsp; (except for tabs)
