...
/* {{{ Initial comments */
/*
 * $LastChangedDate: 2009-02-10 10:41:42 +0100 (Di, 10. Feb 2009) $
...
static u_char *generate_csrf_key(u_char *user,int previous) {
  ...
  u_char *buf = cf_alloc(NULL,1,maxlen,CF_ALLOC_MALLOC);
  u_char *result_buf = cf_alloc(NULL,1,EVP_MAX_MD_SIZE,CF_ALLOC_MALLOC);
  ...
  free(buf);
  buf = cf_alloc(NULL,1,2*result_len+1,CF_ALLOC_MALLOC);
...
static int check_csrf_key(u_char *user,char *csrf_token_user) {
  ...
  if(cf_strcmp(csrf_token_generated,csrf_token_user) == 0) {
...
/* {{{ flt_csrf_newpost */
#ifdef CF_SHARED_MEM
int flt_csrf_newpost(cf_hash_t *head,cf_configuration_t *cfg,cf_message_t *p,cf_cl_thread_t *thr,void *ptr,int sock,int mode)
#else
int flt_csrf_newpost(cf_hash_t *head,cf_configuration_t *cfg,cf_message_t *p,cf_cl_thread_t *thr,int sock,int mode)
#endif
{
