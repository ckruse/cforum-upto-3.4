...
int flt_votingvariables_execute_filter(t_cf_hash *head,t_configuration *dc,t_configuration *vc,t_cl_thread *thread,t_cf_template *tpl) {
  ...
  t_name_value *v = cfg_get_first_value(dc,"VoteURL"),*cs = cfg_get_first_value(dc,"ExternCharset");
  size_t len;
  ...
  if(flt_vv_Config.activate) {
    ...
    if(UserName) {
      ...
      tmp = advanced_get_link(v->values[0],thread->tid,thread->threadmsg->mid,"a=good",6,&len);
      cf_set_variable(tpl,cs,"vote_link_good",tmp,len,1);

      tmp = advanced_get_link(v->values[0],thread->tid,thread->threadmsg->mid,"a=bad",5,&len);
    ...
    if(flt_vv_Config.show_votes) {
      ...
      len = snprintf(buff,512,"%lu",thread->threadmsg->votes_good);
      tpl_cf_setvar(tpl,"votes_good",buff,len,0);

      len = snprintf(buff,512,"%lu",thread->threadmsg->votes_bad);
...
int flt_votingvariables_handle(t_configfile *cfile,t_conf_opt *opt,u_char **args,int argnum) {
