...
static cf_string_t flt_scoring_str        = CF_STRING_INITIALIZER;
static int      flt_scoring_number     = 0;

static cf_array_t flt_scoring_ary         = CF_ARRAY_INITIALIZER(0,NULL);
...
^[[:space:]]*Field:[[:space:]]*[a-zA-Z]+\n? {
  ...
  cf_str_char_set(&flt_scoring_str,ptr,len);
...
/* {{{ flt_scoring_execute */
int flt_scoring_execute(cf_hash_t *head,cf_configuration_t *dc,cf_configuration_t *vc,message_t *msg,u_int64_t tid,int mode) {
  int res = 0;
  size_t i;
  ...
  if(flt_scoring_ary.elements) {
    for(i=0;i<flt_scoring_ary.elements;i++) {
      flt = cf_array_element_at(&flt_scoring_ary,i);
...
/* {{{ flt_scoring_parse */
int flt_scoring_parse(cf_configfile_t *cf,cf_conf_opt_t *opt,const u_char *context,u_char **args,size_t argnum) {
  u_char *error;
  ...
  cf_array_init(&flt_scoring_ary,sizeof(filter),NULL);
  yybuff = yy_scan_string(args[0]);

  flt_scoring_number = 1;
  cf_str_init(&flt_scoring_str);
  ...
  do {
    ...
    switch(ret) {
      ...
      case FLT_SCORING_REGEX:
        ...
        cf_array_push(&flt_scoring_ary,&filter);
        break;
  ...
  cf_str_cleanup(&flt_scoring_str);
...
/* {{{ flt_scoring_cols */
int flt_scoring_cols(cf_configfile_t *cf,cf_conf_opt_t *opt,const u_char *context,u_char **args,size_t argnum) {
  u_char *arg;
  size_t len;
  u_char *ptr, *col;
  u_char tmp[3];
...
/* {{{ flt_scoring_hide */
int flt_scoring_hide(cf_configfile_t *cf,cf_conf_opt_t *opt,const u_char *context,u_char **args,size_t argnum) {
  if(flt_scoring_fn == NULL) flt_scoring_fn = cf_hash_get(GlobalValues,"FORUM_NAME",10);
  if(!context || cf_strcmp(flt_scoring_fn,context) != 0) return 0;
...
/* {{{ flt_scoring_vals */
int flt_scoring_vals(cf_configfile_t *cf,cf_conf_opt_t *opt,const u_char *context,u_char **args,size_t argnum) {
  if(flt_scoring_fn == NULL) flt_scoring_fn = cf_hash_get(GlobalValues,"FORUM_NAME",10);
  if(!context || cf_strcmp(flt_scoring_fn,context) != 0) return 0;
...
/* {{{ flt_scoring_ignore */
int flt_scoring_ignore(cf_configfile_t *cf,cf_conf_opt_t *opt,const u_char *context,u_char **args,size_t argnum) {
  if(flt_scoring_fn == NULL) flt_scoring_fn = cf_hash_get(GlobalValues,"FORUM_NAME",10);
  if(!context || cf_strcmp(flt_scoring_fn,context) != 0) return 0;
...
void flt_scoring_finish(void) {
  ...
  for(i=0;i<flt_scoring_ary.elements;i++) {
    f = cf_array_element_at(&flt_scoring_ary,i);
    pcre_free(f->regex_extra);
    pcre_free(f->regex);
  }

  cf_array_destroy(&flt_scoring_ary);
}
/* }}} */

cf_conf_opt_t flt_scoring_config[] = {
  { "ScoringFilter",          flt_scoring_parse,  CF_CFG_OPT_CONFIG|CF_CFG_OPT_USER, NULL },
  { "ScoringHideScore",       flt_scoring_hide,   CF_CFG_OPT_CONFIG|CF_CFG_OPT_USER, NULL },
  { "ScoringMaxValue",        flt_scoring_vals,   CF_CFG_OPT_CONFIG|CF_CFG_OPT_USER, NULL },
  { "ScoringMinValue",        flt_scoring_vals,   CF_CFG_OPT_CONFIG|CF_CFG_OPT_USER, NULL },
  { "ScoringMaxColor",        flt_scoring_cols,   CF_CFG_OPT_CONFIG|CF_CFG_OPT_USER, NULL },
  { "ScoringNormalColor",     flt_scoring_cols,   CF_CFG_OPT_CONFIG|CF_CFG_OPT_USER, NULL },
  { "ScoringMinColor",        flt_scoring_cols,   CF_CFG_OPT_CONFIG|CF_CFG_OPT_USER, NULL },
  { "ScoringIgnoreNoneMatch", flt_scoring_ignore, CF_CFG_OPT_CONFIG|CF_CFG_OPT_USER, NULL },
  { NULL, NULL, 0, NULL }
};

cf_handler_config_t flt_scoring_handlers[] = {
  { VIEW_LIST_HANDLER,    flt_scoring_execute },
  { 0, NULL }
};

cf_module_config_t flt_scoring = {
  MODULE_MAGIC_COOKIE,
  flt_scoring_config,
