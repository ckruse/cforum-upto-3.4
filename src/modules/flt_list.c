...
int flt_list_execute_filter(t_cf_hash *head,t_configuration *dc,t_configuration *vc,t_cl_thread *thread,t_cf_template *tpl) {
  ...
  t_name_value *ps,*cs,*st,*qc,*ms,*ss,*locale,*df,*dft,*rm = cfg_get_first_value(vc,forum_name,"ReadMode"),*lt;
  ...
  if(cf_strcmp(st->values[0],"none") != 0) {
    cf_gen_threadlist(thread,head,&threadlist,st->values[0],lt->values[0]);
